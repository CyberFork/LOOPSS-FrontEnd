<template>
  <div class="trust">
    <a-box class="search-wrap">
      ...
      <a-input-search v-model="search.inputVal" :placeholder="$t('trust.placeholder')" enter-button size="large" @change="onSearch" :loading="search.loading" allowClear />
      <div class="list-wrap" v-if="search.inputVal">
        <a-list class="invite-list" :loading="search.loading" :data-source="search.list">
          <a-list-item slot="renderItem" slot-scope="item, index" :key="index">
            <div class="list-item-wrap">
              <a-space size="large">
                <a-text>{{item.address}}</a-text>
                <a-tag :color="item.trustType | formatTrustType">{{item.trustType | formatTrustType('desc')}}</a-tag>
              </a-space>
              <a-space class="actions f-r">
                <a-icon v-if="item.trustType !== 2" class="pointer" type="plus-circle" @click="addTrust" />
                <a-icon v-else class="pointer" type="minus-circle" @click="minusTrust" />
              </a-space>
            </div>
          </a-list-item>
        </a-list>
    ...
    <a-box class="your-trusts-container" >
      <div class="title"><a-text strong>{{$t('trust.yourTrusts')}} ({{yourTrusts.total}})</a-text></div>
      <a-spin :spinning="yourTrusts.loading && !yourTrusts.busy">
        <div class="content" v-infinite-scroll="getMyTrusts" :infinite-scroll-disabled="yourTrusts.busy" :infinite-scroll-distance="10">
            ...
            <a-list-item slot="renderItem" slot-scope="item, index" :key="index">
              <div class="list-item-wrap">
                ...
                <a-space class="actions">
                  <a-icon class="pointer" type="copy" @click="copyFn(item.address)" />
    ...
    methods: {
      ...
      getYourTrusts() {
        ...
        setTimeout(() => {
          this.yourTrusts.loading = false
          this.yourTrusts.busy = false
          this.yourTrusts.list = this.yourTrusts.list.concat(this.yourTrusts.list)
        }, 2000)
      },
      removeSearchTimer() {
